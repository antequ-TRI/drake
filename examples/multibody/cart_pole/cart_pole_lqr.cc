...
void DoMain() {
  ...
  DRAKE_DEMAND(cp->num_actuated_dofs() == 2);
  ...
  {
    Eigen::VectorXd f0 = Eigen::VectorXd::Zero(2);
    Eigen::VectorXd phases(2);
    phases << M_PI_2, M_PI_2;  
    const auto& constant_force = *builder.AddSystem<systems::Sine>( u0, f0, phases );
    builder.Connect(constant_force.get_output_port(0), cp->get_actuation_input_port());
  }
