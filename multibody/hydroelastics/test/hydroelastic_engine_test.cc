...
class SphereVsPlaneTest : public ::testing::Test {
 ...
 protected:
  ...
  double height_{0.04};
  double radius_{0.05};  // consisten with the *.sdf file.
};
...
TEST_F(SphereVsPlaneTest, VerifyModelSizeAndResults) {
  EXPECT_EQ(plant_->num_bodies(), 3);  // Including the "world" body.

  // Models are created not until the first query passing a context.
